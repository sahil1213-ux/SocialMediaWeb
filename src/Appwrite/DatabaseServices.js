...
class DatabaseServices {
    ...
    async getCurrentUser(currentAccount) {
        return await databases.listDocuments(appwriteConfig.databaseID, appwriteConfig.usersCollectionID, [Query.equal("accountID", (await currentAccount.get()).$id)]);
    }
    // ============================== CREATE POST
    ...
    // ============================== GET INFINITE POSTS
    async getInfinitePosts({ pageParam }) {
    ...
    async getUserPosts(userId) {
        if (!userId)
            return;
        try {
            const post = await databases.listDocuments(appwriteConfig.databaseID, appwriteConfig.postsCollectionID, [Query.equal("creator", userId), Query.orderDesc("$createdAt")]);
            if (!post)
                throw Error;
            return post;
        }
